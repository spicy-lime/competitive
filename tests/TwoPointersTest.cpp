...
TEST(targetPair, testHash)
{
    ...
    auto res = pairWithTargetSumHash(std::get<0>(tests[0]), std::get<1>(tests[0]));
    EXPECT_EQ(res, std::make_pair(1, 3));

    res = pairWithTargetSumHash(std::get<0>(tests[1]), std::get<1>(tests[1]));
    EXPECT_EQ(res, std::make_pair(0, 2));
